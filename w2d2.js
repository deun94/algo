...
//keys and values
//don't use pop or push since this is not ordered
...
for (var i=0; i<pokemon.length; i++){
    if(pokemon[i].types.length == 1 && pokemon[i].types == "poison"){
        var newname = ""
        for (var j=pokemon[i].name.length-1; j<=pokemon[i].name.length; j--){
            newname += pokemon[i].name[j];
            if (newname.length ==pokemon[i].name.length){
                console.log(newname)
                break;
            }
        }
